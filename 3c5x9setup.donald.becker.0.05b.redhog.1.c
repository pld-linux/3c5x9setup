/* 3c5x9setup.c: Setup program for 3Com EtherLink III ethercards.
   ...
   Copyright 1994-1999 by Donald Becker <becker@cesdis.gsfc.nasa.gov>
   Modified 2000 by RedHog (Egil Möller) <redhog@lysator.liu.se>
...
"3c5x9setup.c:donald.becker.0.05b.redhog.1 2/5/00 RedHog (Egil Möller) <redhog@lysator.liu.se>\n";
static char *usage_msg =
"Usage: 3c5x9setup [-aEfFsvVw] [-p <IOport>] [-F 10baseT|10base2|AUI>] [-Q <IRQ>] [-d <filename>] [-r <filename>] [-u <UMID>] [-m <MAC address>]\n";
...
struct option longopts[] = {
 /* { name  has_arg  *flag  val } */
	...
	{"dump", 1, 0, 'd'},        /* Dump EEPROM content to file */
	{"restore", 1, 0, 'r'},     /* Restore EEPROM content from fale */
    {"new-hw", 1, 0, 'm'},      /* Sets the MAC adress of the card */
    {"new-muid", 1, 0, 'u'},    /* Sets the Unique identifier of the card */
	{ 0, 0, 0, 0 }
};
...
main(int argc, char **argv)
{
	...
	char *dumpcard = NULL;
	char *restorecard = NULL;
    char *new_hw = NULL;
    char *new_umid = NULL;
	int c, longind, i, j, saved_window;
	extern char *optarg;

	while ((c = getopt_long(argc, argv, "aDeEfF:hi:p:P:Q:svVwX:d:r:o:m:",
							longopts, &longind))
		   != -1)
		switch (c) {
    ...
    if (restorecard)
	 {
	  ...
	  if ((fd = open(restorecard, O_RDONLY)))
	   {
		for (i = 0; i < 16; i++)
		 {
		  while ((ret = read(fd, (char *) (eeprom_contents + i), 1)) == 0);
		  if (ret < 0)
		   break;
		  while ((ret = read(fd, ((char *) (eeprom_contents + i)) + 1, 1)) == 0);
		  if (ret < 0)
		   break;
		 }
		if (ret < 0)
		 {
		  perror("Unable to read EEPROM image from file.");
		  exit(1);
		 }
		else
		 fprintf(stderr, "EEPROM image read from file '%s'.\n", restorecard);
		close(fd);
       }
	  else
	   {
	  ...
	  if (do_write_eeprom)
	   {
        for (i = 0; i < 16; i++)
         write_eeprom(ioaddr, i, eeprom_contents[i]);
		fprintf(stderr, "EEPROM image written to card.\n");
	   }
	  else
	   fprintf(stderr, "Specify -w to write image to card.\n");
	 }

	{
		...
		if (new_hw)
 		 {
          unsigned short mac_address_tmp[6];
          unsigned char  mac_address[6];

          sscanf(new_hw, "%X:%X:%X:%X:%X:%X",
				 (unsigned int* ) (mac_address_tmp + 0), (unsigned int* ) (mac_address_tmp + 1),
				 (unsigned int* ) (mac_address_tmp + 2), (unsigned int* ) (mac_address_tmp + 3),
				 (unsigned int* ) (mac_address_tmp + 4), (unsigned int* ) (mac_address_tmp + 5));

          for (i = 0; i < 6; i++)
           mac_address[i] = (unsigned char) mac_address_tmp[i^1];

          do_update(eeprom_contents, PhysAddr01, "MACAddress", ((unsigned short*) mac_address)[0]);
		  do_update(eeprom_contents, PhysAddr23, "MACAddress", ((unsigned short*) mac_address)[1]);
		  do_update(eeprom_contents, PhysAddr45, "MACAddress", ((unsigned short*) mac_address)[2]);
		 }
		...
		if (new_umid)
          ...
          do_update(eeprom_contents, AltPhysAddr01, "UMID", ((unsigned short*) mac_address)[0]);
		  do_update(eeprom_contents, AltPhysAddr23, "UMID", ((unsigned short*) mac_address)[1]);
		  do_update(eeprom_contents, AltPhysAddr45, "UMID", ((unsigned short*) mac_address)[2]);
		 }
    ...
    if (dumpcard)
	 {
	  ...
	  if ((fd = open(dumpcard, O_WRONLY | O_CREAT | O_TRUNC)))
	   {
		for (i = 0; i < 16; i++)
		 {
		  while ((ret = write(fd, (char *) (eeprom_contents + i), 1)) == 0);
		  if (ret < 0)
		   break;
		  while ((ret = write(fd, ((char *) (eeprom_contents + i)) + 1, 1)) == 0);
		  if (ret < 0)
		   break;
		 }
		if (ret < 0)
		 perror("Unable to write EEPROM image to file.");
		else
		 fprintf(stderr, "EEPROM image written to file '%s'.\n", dumpcard);
	   }
	  else
       perror("Unable to write EEPROM image to file.");
      close(fd);
...
static void print_eeprom(unsigned short *eeprom_contents)
{
	...
	printf("  3Com Node Address (UMID) ");
	for (i =  0; i < 6; i++)
		printf("%2.2X%s", ((unsigned char *)(eeprom_contents + AltPhysAddr01))[i^1], i < 5 ? ":" : " (used as a unique ID only).\n");
	printf("  OEM Station address (MAC) ");
	for (i =  0; i < 6; i++)
		printf("%2.2X%s", ((unsigned char *)(eeprom_contents + PhysAddr01))[i^1], i < 5 ? ":" : " (used as the ethernet address).\n");
	...
	printf("  Manufacture date (MM/DD/YY) %d/%d/%d, division %c,"
		   " product %c%c.\n", (ee[4] >> 5) & 15,
		   ee[4] & 31, ee[4] >> 9, ((unsigned char *)(eeprom_contents + PhysAddr01))[10], ((unsigned char *)(eeprom_contents + PhysAddr01))[12], ((unsigned char *)(eeprom_contents + PhysAddr01))[13]);
